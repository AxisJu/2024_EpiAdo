# ==============================================================================
# Script Name: FigS5C_Metabolic_Pathways_ADORA1_Correlation.R
# Description: Correlation between ADORA1 expression and 12 metabolic/stress pathways
#              in ADORA1^High and ADORA1^Low Epilepsy glutamatergic neurons.
...
out_csv_data <- "../sourcedata/SuppFig5/FigS5C_Metabolic_Pathways_ADORA1_Correlation.csv"
out_csv_stat <- "../sourcedata/SuppFig5/FigS5C_Metabolic_Pathways_Statistics.csv"
check_plot_path <- "../sourcedata/check_plots/check_FigS5C_Metabolic_Pathways_ADORA1_Correlation.png"
...
for (pw in s5c_names) {
  for (grp in c("ADORA1^High Epilepsy", "ADORA1^Low Epilepsy")) {
    sub_d <- s5c_df %>% filter(Pathway == pw, Group == grp)
...
plot_single_pathway <- function(pw_name) {
  d_pw <- s5c_df %>% filter(Pathway == pw_name)
  ...
  # Center separator: vertical dashed line with zero margin
  sep_line <- ggplot() +
...
plot_list <- lapply(s5c_names, plot_single_pathway)
combined_grid <- wrap_plots(plot_list, ncol = 4, nrow = 3)
...
final_c_plot <- (y_axis_label | combined_grid) + plot_layout(widths = c(0.025, 1))
final_c_plot <- (final_c_plot / bottom_bar) + plot_layout(heights = c(1, 0.05))

ggsave(check_plot_path, plot = final_c_plot, width = 11.5, height = 5.6, dpi = 300)
